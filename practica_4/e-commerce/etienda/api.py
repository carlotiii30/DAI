...
# - - - - -  - - - - - - - RATING - - - - - - - - - - - - - - - -
@api.put(
    "/productos/{id}/rating/{newRating}",
    tags=["TIENDA DAI"],
    response={202: ProductSchema, 404: ErrorSchema},
)
def Actualiza_rating(request, id: str, newRating: int):
    try:
        # Buscar desde el string id
        resu = productos_collection.find_one({"_id": ObjectId(id)})

        if resu:
            # Actualizar la calificación y el recuento de votos
            nuevaPuntuacionMedia = (resu["rating"]["rate"] * resu["rating"]["count"] + newRating) / (resu["rating"]["count"] + 1)
            resu["rating"]["rate"] = round(nuevaPuntuacionMedia, 2)
            resu["rating"]["count"] += 1
            ...
            logger.info(f"Nuevo rating: {resu['id']}")
            return 202, resu
        else:
            return 404, {"message": "no encontrado"}
    except Exception as e:
        logger.info(f"Error al actualizar rating: {e}")
        return 404, {"message": "error al actualizar rating"}
...
def Busca_producto(request, palabra: str):
    try:
        # Construir una expresión regular de palabra completa
        regex_pattern = re.compile(rf'\b{re.escape(palabra)}\b', re.IGNORECASE)

        # Buscar en el título o la descripción usando la expresión regular
        resu = productos_collection.find(
            {"$or": [{"title": regex_pattern}, {"description": regex_pattern}]}
        )
        ...
        logger.info(f"Productos encontrados")
        return 200, resu
    except Exception as e:
        logger.info(f"Error al buscar productos: {e}")
        return 404, {"message": "Error al buscar productos"}
...
def Lista_productos_categoria(request, categoria: str):
    try:
        # Obtener los productos desde la base de datos
        resu = productos_collection.find({"category": categoria})
        ...
        logger.info(f"Productos de una categoria listados")
        return 200, resu
    except Exception as e:
